<template>
  <div class="checklist-comunicacion-radial">
    <v-row dense class="mb-3 header-fields">
      <v-col
      ...
      >
        <v-text-field
          ...
          hide-details
          :disabled="disabled"
          @update:model-value="(value) => setDato(field.key, field.type === 'date' ? normalizeDateForInput(value) : value)"
        />
      </v-col>
      <v-col cols="12" sm="6">
        <v-text-field
...
const dmhDatosFields = [
  { key: 'trabajadorInspecciona', label: 'Trabajador que inspecciona' },
  { key: 'especialidad', label: 'Especialidad' },
  { key: 'tipoRadio', label: 'Tipo de radio' },
  { key: 'marcaModelo', label: 'Marca y modelo' },
  { key: 'firmaTrabajador', label: 'Firma trabajador' }
]

const spotDatosFields = [
  { key: 'trabajadorInspecciona', label: 'Trabajador que inspecciona' },
  { key: 'especialidad', label: 'Especialidad' },
  { key: 'firmaTrabajador', label: 'Firma trabajador' }
]

const datosFields = computed(() => isSpotCodelco.value ? spotDatosFields : dmhDatosFields)

const options = [
  { value: 'si', label: 'SI', color: 'success' },
  { value: 'no', label: 'NO', color: 'error' },
  { value: 'na', label: 'N/A', color: 'blue-grey' }
]
...
const dmhDefaultRows = [
  ...
  'Se encuentran en buen estado botones, perillas y antena de la radio.',
  ...
  'Cantidad de radios de comunicacion suficientes en la tarea desarrollada.',
  'Condiciones de instalacion de radios bases, repetidores y controladores.',
  'Otros.'
]

const spotDefaultRows = [
  'Radio transmisor se encuentra con bateria cargada.',
  'Se verifico que el canal de comunicacion este libre de interferencias e intervenciones.',
  'La prueba de comunicacion entre emisor y receptor con radio transmisor es efectiva.',
...
const isSpotCodelco = computed(() => {
  return String(attrRef.value?.type || '').trim().toLowerCase() === 'checklistspotcomunicacionradialcodelco'
})

const defaultRows = computed(() => isSpotCodelco.value ? spotDefaultRows : dmhDefaultRows)
...
const fechaTerminoModel = computed(() => {
  return normalizeDateForInput(attrRef.value?.fechaTermino) || getDatePlusDays(fechaInicioModel.value, 6)
})
...
function ensureDatos() {
  ...
  datosFields.value.forEach((field) => {
...
function getDatePlusDays(value, daysToAdd) {
...
function onFechaInicioChange(value) {
  attrRef.value.fechaInicio = normalizeDateForInput(value) || todayIso()
  attrRef.value.fechaTermino = getDatePlusDays(attrRef.value.fechaInicio, 6)
  renderTick.value += 1
  emit('change')
}

function onFechaTerminoChange(value) {
  attrRef.value.fechaTermino = normalizeDateForInput(value) || getDatePlusDays(attrRef.value.fechaInicio, 6)
  renderTick.value += 1
  emit('change')
}
...
function ensureBody() {
  ...
  attrRef.value.fechaTermino = normalizeDateForInput(attrRef.value.fechaTermino) || getDatePlusDays(attrRef.value.fechaInicio, 6)
  ensureColorCoding()
  ...
  if (!attrRef.value.body.length && Array.isArray(attrRef.value.checkBoby) && attrRef.value.checkBoby.length) {
    attrRef.value.body = attrRef.value.checkBoby.map((item, index) => ({
      id: item?.id || `${index + 1}`,
      kind: 'item',
      label: item?.label || defaultRows.value[index] || `Item ${index + 1}`,
      days: item?.days && typeof item.days === 'object' ? { ...item.days } : emptyDays()
    }))
  }

  if (!attrRef.value.body.length) {
    attrRef.value.body = defaultRows.value.map((label, index) => ({
      id: `${index + 1}`,
      kind: 'item',
      label,
      days: emptyDays()
...
function normalizeColorCoding(saved) {
  ...
  const hasManualColors = source.coloresManuales === true

  const meses = months.reduce((acc, month) => {
    const legacyColor = hasManualColors ? legacyRows.find((row) => row?.meses?.[month.key])?.key || '' : ''
    const value = hasManualColors ? (sourceMonths[month.key] || legacyColor || '') : ''
  ...
  return {
    ...
    coloresManuales: hasManualColors,
    meses
  }
}
...
function normalizeRow(row, index) {
  const normalized = {
    id: row?.id || `${index + 1}`,
    kind: row?.kind || 'item',
    label: row?.label || defaultRows.value[index] || `Item ${index + 1}`,
...
function setColorObservation(value) {
  ...
  attrRef.value.codificacionColores = {
    ...attrRef.value.codificacionColores,
    coloresManuales: attrRef.value.codificacionColores?.coloresManuales === true,
    observacion: value || ''
  }
  renderTick.value += 1
  emit('change')
