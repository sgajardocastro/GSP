...
const viajeModel = {
  ...
  async getViajePorToken(token) {
    ...
    // 1.2 Si no existe aún la fila en tequ_log_desplazamiento, parsear el token para resolver proyecto y equipo
    // Formato de token: vj_{id_proyecto}_{id_equipo}_{hash}
    const tokenParts = token.split('_');
    if (tokenParts.length >= 3 && tokenParts[0] === 'vj') {
      const id_proyecto = parseInt(tokenParts[1], 10);
      const id_equipo = parseInt(tokenParts[2], 10);
  ...
  async registrarSalida({ token_viaje, id_proyecto, id_equipo, id_user_chofer, km_inicial, horometro_inicial, foto_salida, pin_hash, latitud, longitud }) {
    ...
    if (checkRes.rows.length > 0) {
      ...
      // Parsear token si no se enviaron ids explícitos
      if (!idPry || !idEq) {
        const parts = token_viaje.split('_');
        if (parts.length >= 3 && parts[0] === 'vj') {
          idPry = parseInt(parts[1], 10);
          idEq = parseInt(parts[2], 10);
        }
      }
