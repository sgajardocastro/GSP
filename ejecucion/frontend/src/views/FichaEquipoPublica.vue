  ...
  <div class="min-h-screen bg-[#0a0a0a] text-foreground flex flex-col font-sans relative overflow-hidden">
    ...
    <main class="flex-1 w-full max-w-md mx-auto p-4 sm:p-6 relative z-10 flex flex-col gap-6" v-if="!loading && valid">
      ...
      <header class="flex flex-col items-center pt-8 pb-4">
        <div class="w-24 h-24 rounded-2xl bg-gradient-to-br from-emerald-500/20 to-zinc-800/50 flex items-center justify-center border border-emerald-500/20 mb-4 shadow-[0_0_30px_rgba(16,185,129,0.15)] relative">
          <Truck class="w-10 h-10 text-emerald-400" />
          <div class="absolute -bottom-2 -right-2 bg-emerald-500 w-6 h-6 rounded-full flex items-center justify-center border-2 border-[#0a0a0a]">
            <Check class="w-3.5 h-3.5 text-black" />
          </div>
        </div>
        
        <div class="bg-white/10 px-4 py-1.5 rounded-lg border border-white/20 mb-3 backdrop-blur-sm">
          <h1 class="text-2xl font-black text-white tracking-widest">{{ equipo.patente || $route.params.patente }}</h1>
        </div>
        
        <!-- Lista de Especificaciones Técnicas (1 por línea) -->
        <div class="w-full flex flex-col gap-2 mb-2">
          <div v-for="(spec, idx) in equipo.specs" :key="idx" class="bg-white/5 rounded-xl border border-white/10 p-3 flex justify-between items-center">
      ...
      <section class="flex-1 flex flex-col gap-3">
        <h2 class="text-xs font-bold text-zinc-500 uppercase tracking-widest mb-2 px-1">Documentación Legal</h2>
      ...
      <section class="flex-1 flex flex-col gap-3 mt-4">
        <h2 class="text-xs font-bold text-zinc-500 uppercase tracking-widest mb-2 px-1">Documentación General</h2>
        
        <div 
          v-for="(doc, index) in docGenerales" 
          :key="'gen-'+index"
          class="bg-zinc-900/50 backdrop-blur-md rounded-2xl border border-white/5 p-4 flex items-center gap-4 transition-all duration-300 hover:bg-zinc-800/50 group"
        >
          <!-- Icono tipo de documento -->
          <div class="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" 
               :class="doc.estado === 'Vigente' ? 'bg-blue-500/10 text-blue-400' : (doc.estado === 'Por Vencer' ? 'bg-amber-500/10 text-amber-400' : 'bg-red-500/10 text-red-400')">
            <FileText class="w-5 h-5" />
          </div>
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="flex items-center gap-2 mt-1">
              <span class="w-1.5 h-1.5 rounded-full" 
                    :class="doc.estado === 'Vigente' ? 'bg-blue-500' : (doc.estado === 'Por Vencer' ? 'bg-amber-500' : 'bg-red-500')"></span>
              <p class="text-xs text-zinc-400 truncate">Vence: {{ doc.fechaVencimiento }}</p>
            </div>
          </div>
    ...
    <div v-else-if="loading" class="flex-1 flex flex-col items-center justify-center">
      <div class="relative w-16 h-16 flex items-center justify-center mb-6">
        ...
        <Truck class="w-6 h-6 text-emerald-500" />
      </div>
      <p class="text-sm font-medium text-zinc-400 animate-pulse tracking-wide">Validando equipo...</p>
    </div>
...
import { Truck, Check, FileText, Download, Zap, ShieldAlert } from 'lucide-vue-next'
...
const docGenerales = ref([])

onMounted(async () => {
  const patente = route.params.patente

  try {
    // 1. Obtener la flota completa desde GSP
    const res = await apiAxios.get('/tequ-equipos')
    const fleetItems = res.data.data || []
    
    // Buscar el equipo por patente (limpiando espacios)
    const item = fleetItems.find(i => (i.patente || '').trim().toLowerCase() === patente.toLowerCase())
    ...
    equipo.value = {
      ...
      marca: item.nombre_marca || item.marca || 'S/I',
      modelo: item.nombre_modelo || item.modelo || 'S/I',
      specs: [
        { label: 'Año', value: item.ano_fabricacion || 'S/I' },
        { label: 'Categoría', value: item.nombre_categoria || item.tipo_equipo || 'S/I' },
        { label: 'Subcategoría', value: item.nombre_subcategoria || 'S/I' },
        { label: 'Capacidad Máxima', value: item.capacidad_maxima ? `${item.capacidad_maxima} ${item.unidad_capacidad || 'T'}` : 'S/I' },
        { label: 'Número de Serie', value: item.numero_serie || 'S/I' },
        { label: 'Estado', value: item.estado || 'S/I' }
      ]
    }

    // 2. Obtener certificados desde GSP
    const certsRes = await apiAxios.get(`/tequ-equipos/${eqId}/certificados`)
    ...
    documentos.value = legalDocs
    docGenerales.value = generalDocs
