<template>
  <section class="space-y-6 pb-12 animate-in fade-in duration-500 overflow-y-auto custom-scroll h-full">
    ...
    <!-- SECTION 1: ANÁLISIS DE SEGURIDAD (COLAPSABLE) -->
    <div class="glass-card-design rounded-2xl border border-slate-800 bg-[#16191f] overflow-hidden shadow-xl transition-all duration-300">
      ...
      >
        ...
        <div class="flex items-center gap-3">
          ...
          <h2 class="text-[11px] font-black uppercase tracking-[0.2em] text-slate-200">Análisis de Seguridad</h2>
        </div>
      </div>
    ...
    <div class="glass-card-design rounded-2xl border border-slate-800 bg-[#16191f] overflow-hidden shadow-2xl transition-all duration-300">
      ...
      >
        ...
        <div class="flex items-center gap-3">
          ...
          <h2 class="text-[11px] font-black uppercase tracking-[0.2em] text-slate-200">Planificación y Registro de Seguridad</h2>
        </div>
      </div>
      ...
      <div class="collapse-transition" :class="{'collapsed': !sections.plan}">
        <div class="p-6 pt-0 border-t border-slate-800/50">
            ...
            <div class="space-y-2">
              ...
              <div class="relative">
                <select v-model="form.cliente" class="w-full h-11 bg-[#0a0a0c] border border-slate-800 rounded-xl px-4 text-xs focus:border-blue-500 outline-none text-slate-400 appearance-none">
                  ...
                  <option v-for="cliente in clientesFiltrados" :key="cliente.id_empresa" :value="cliente.id_empresa">
            ...
            <div class="space-y-2">
              ...
              <div class="relative">
                <select v-model="form.proyecto" class="w-full h-11 bg-[#0a0a0c] border border-slate-800 rounded-xl px-4 text-xs focus:border-blue-500 outline-none text-slate-400 appearance-none" :disabled="!form.cliente || loadingProyectos">
                  ...
                  <option v-for="proyecto in proyectosFiltrados" :key="proyecto.id_proyecto" :value="proyecto.id_proyecto">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[1fr_1fr_auto] gap-5 mt-6 items-end">
            <div class="space-y-2">
              <label class="text-[9px] font-black uppercase tracking-[0.2em] text-slate-500 tracking-widest">Inicio de Inspección</label>
              <div class="relative">
            ...
            <div class="space-y-2">
              <label class="text-[9px] font-black uppercase tracking-[0.2em] text-slate-500 tracking-widest">Fin de Inspección</label>
              <div class="relative">
            ...
            >
              ...
              {{ creandoSurvey ? 'Creando...' : 'Crear Inspección de Seguridad' }}
            </button>
          </div>
...
const cargarRolesSesion = async () => {
  try {
    ...
    console.error('[InspeccionSeguridad] Error cargando roles de sesión:', error)
  }
}
...
const form = reactive({
  cliente: '',
  proyecto: '',
  familia: 'Inspección de Seguridad',
  protocolo: 'Inspección de Seguridad',
...
// Conteo de Condiciones Inseguras: surveys visibles en la tabla que están en el set de inseguras
const condicionesInsegurasCount = computed(() => {
  const ids = tablaInspeccionesRef.value?.insegurasIds
...
const kpisDinamicos = computed(() => [
  ...
  { label: 'Cond. Inseguras', value: condicionesInsegurasCount.value, icon: AlertTriangle, color: 'red' }
])
...
const crearSurvey = async () => {
  ...
  try {
    const response = await apiAxios.get('/servicio/leanglobal/obtenerTemplates', {
      params: { id_tipo_srv: 8 }
    })
    const templates = Array.isArray(response.data) ? response.data : []
    const template = templates.find(t => Number(t.id_template) === 80)
    if (!template) {
      throw new Error('No se encontró el template 80 para Inspección de Seguridad.')
    }
    ...
    mensaje.value = creado?.id_survey
      ? `Inspección creada correctamente (#${creado.id_survey}).`
...
onMounted(async () => {
  ...
  // Autoseleccionar Transmac si no hay auto-selección por rol
  if (!form.cliente) {
